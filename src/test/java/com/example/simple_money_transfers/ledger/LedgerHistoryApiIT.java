...
class LedgerHistoryApiIT extends AbstractIntegrationTest {
  ...
  void returnsEveryEntryTouchingTheAccountAcrossPages() throws Exception {
    ...
    transferService.execute(
        new com.example.simple_money_transfers.transfer.TransferCommand(
            account.getId(),
            other.getId(),
            new BigDecimal("10.00"),
            "USD",
            com.example.simple_money_transfers.transfer.TransferKind.TRANSFER,
            null));
    transferService.execute(
        new com.example.simple_money_transfers.transfer.TransferCommand(
            account.getId(),
    ...
    do {
      MvcResult result =
          mockMvc
              .perform(
                  get("/api/v1/accounts/" + account.getId() + "/entries")
  ...
  void unknownAccountReturns404() throws Exception {
    mockMvc
        .perform(
            get("/api/v1/accounts/" + UUID.randomUUID() + "/entries")
                .header(ApiKeyAuthFilter.HEADER_NAME, VALID_KEY))
  ...
  void missingApiKeyIsRejected() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/accounts/" + account.getId() + "/entries"))
