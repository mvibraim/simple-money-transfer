...
class LedgerHistoryApiIT extends AbstractIntegrationTest {
	...
	void returnsEveryEntryTouchingTheAccountAcrossPages() throws Exception {
		Account account = accountRepository.save(
				new Account("H1", "History", AccountType.CUSTOMER, "USD", AccountStatus.ACTIVE));
		Account other = accountRepository.save(
				new Account("H2", "Other", AccountType.CUSTOMER, "USD", AccountStatus.ACTIVE));

		transferService.deposit(account.getId(), new BigDecimal("100.00"), null);
		transferService.execute(new com.example.simple_money_transfers.transfer.TransferCommand(
				account.getId(), other.getId(), new BigDecimal("10.00"), "USD",
				com.example.simple_money_transfers.transfer.TransferKind.TRANSFER, null));
		transferService.execute(new com.example.simple_money_transfers.transfer.TransferCommand(
				account.getId(), other.getId(), new BigDecimal("5.00"), "USD",
				com.example.simple_money_transfers.transfer.TransferKind.TRANSFER, null));
		...
		int totalPages;
		do {
			MvcResult result = mockMvc.perform(get("/api/v1/accounts/" + account.getId() + "/entries")
	...
	void entriesAreOrderedNewestFirst() throws Exception {
		Account account = accountRepository.save(
				new Account("H3", "History", AccountType.CUSTOMER, "USD", AccountStatus.ACTIVE));
		...
		mockMvc.perform(get("/api/v1/accounts/" + account.getId() + "/entries")
						.header(ApiKeyAuthFilter.HEADER_NAME, VALID_KEY))
				.andExpect(status().isOk())
	...
	void unknownAccountReturns404() throws Exception {
		mockMvc.perform(get("/api/v1/accounts/" + UUID.randomUUID() + "/entries")
						.header(ApiKeyAuthFilter.HEADER_NAME, VALID_KEY))
	...
	void missingApiKeyIsRejected() throws Exception {
		Account account = accountRepository.save(
				new Account("H4", "History", AccountType.CUSTOMER, "USD", AccountStatus.ACTIVE));
