...
class TransferApiIT extends AbstractIntegrationTest {
  ...
  void createThenFetchRoundTrip() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/transfers/" + id).header(ApiKeyAuthFilter.HEADER_NAME, VALID_KEY))
  ...
  void unknownTransferIdReturns404() throws Exception {
    mockMvc
        .perform(
            get("/api/v1/transfers/" + UUID.randomUUID())
                .header(ApiKeyAuthFilter.HEADER_NAME, VALID_KEY))
        .andExpect(status().isNotFound());
  }
  ...
  void missingApiKeyIsRejected() throws Exception {
    mockMvc
        .perform(post("/api/v1/transfers").contentType(MediaType.APPLICATION_JSON).content("{}"))
