...
class FundingApiIT extends AbstractIntegrationTest {
  ...
  void depositMovesTheBalanceAndKeepsTheLedgerBalanced() throws Exception {
    Account account =
        accountRepository.save(
            new Account("D1", "Depositor", AccountType.CUSTOMER, "USD", AccountStatus.ACTIVE));

    mockMvc
        .perform(
            post("/api/v1/accounts/" + account.getId() + "/deposits")
  ...
  void withdrawalBelowBalanceIsRejectedWith422() throws Exception {
    Account account =
        accountRepository.save(
            new Account("W1", "Withdrawer", AccountType.CUSTOMER, "USD", AccountStatus.ACTIVE));

    mockMvc
        .perform(
            post("/api/v1/accounts/" + account.getId() + "/withdrawals")
                .header(ApiKeyAuthFilter.HEADER_NAME, VALID_KEY)
                .header("Idempotency-Key", freshIdempotencyKey())
                .contentType(MediaType.APPLICATION_JSON)
                .content(
  ...
  void depositIntoAnUnsupportedCurrencyIsRejectedWith422() throws Exception {
    Account account =
        accountRepository.save(
            new Account("K1", "Kuwaiti", AccountType.CUSTOMER, "KWD", AccountStatus.ACTIVE));
  ...
  void nonPositiveAmountIsRejectedWith400() throws Exception {
    Account account =
        accountRepository.save(
            new Account("D2", "Depositor", AccountType.CUSTOMER, "USD", AccountStatus.ACTIVE));
  ...
  void missingApiKeyIsRejected() throws Exception {
    Account account =
        accountRepository.save(
            new Account("D3", "Depositor", AccountType.CUSTOMER, "USD", AccountStatus.ACTIVE));
  ...
  void depositThenWithdrawalRoundTripsToZero() throws Exception {
    Account account =
        accountRepository.save(
            new Account("R1", "RoundTrip", AccountType.CUSTOMER, "EUR", AccountStatus.ACTIVE));
