...
/**
 * Every integration test shares one H2 in-memory database for the whole test run
 * ({@code DB_CLOSE_DELAY=-1}), so state that survives past the end of a test method would
 * otherwise leak into every test that runs after it. Cleared here, child tables first, so
 * tests can assume a clean slate - including F11's migration-seeded {@code SYSTEM}
 * accounts, which are deleted along with everything else and then re-seeded, rather than
 * special-cased and preserved. Re-seeding is simpler to reason about globally than
 * requiring every test that creates its own ad-hoc {@code SYSTEM} account to avoid
 * colliding with the seeded ones by currency - that was tried first and turned into
 * exactly the kind of cross-test fragility this class exists to prevent.
 */
@Tag("integration")
@SpringBootTest
