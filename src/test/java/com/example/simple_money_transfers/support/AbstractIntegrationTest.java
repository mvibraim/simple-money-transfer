 ...
 * DB_CLOSE_DELAY=-1}), so state that survives past the end of a test method would
 ...
 * requiring every test that creates its own ad-hoc {@code
 * SYSTEM} account to avoid colliding with the seeded ones by currency - that was tried
 * first and turned into exactly the kind of cross-test fragility this class exists to
 * prevent.
 */
@Tag("integration")
@SpringBootTest
