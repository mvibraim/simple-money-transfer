 ...
 * {@code @AfterEach} from F09 onward - which makes each functional test a partial concurrency test
 * for free.
 */
public final class LedgerInvariants {
  ...
  private static void assertLedgerSumsToZero(JdbcClient jdbcClient) {
    BigDecimal sum =
        jdbcClient
            .sql("SELECT COALESCE(SUM(amount), 0) FROM ledger_entry")
            .query(BigDecimal.class)
            .single();
    assertThat(sum)
        .as("SUM(ledger_entry.amount) must be exactly zero")
  ...
  private static void assertEveryBalanceMatchesItsLedgerEntries(JdbcClient jdbcClient) {
    ...
    assertThat(mismatches)
        .as("every account balance must equal the sum of its own ledger entries")
        .isEmpty();
  }
  ...
  private static void assertNoNegativeCustomerBalance(JdbcClient jdbcClient) {
    var negative =
        jdbcClient
            .sql("SELECT id FROM account WHERE account_type = 'CUSTOMER' AND balance < 0")
            .query()
            .listOfRows();
  ...
  private static void assertExactlyOneDebitAndOneCreditPerTransfer(JdbcClient jdbcClient) {
    ...
    assertThat(malformed)
        .as("every transfer must have exactly one debit and one credit entry")
        .isEmpty();
  }
}
