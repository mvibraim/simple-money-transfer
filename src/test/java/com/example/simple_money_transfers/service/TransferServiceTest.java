...
import com.example.simple_money_transfers.exception.CurrencyMismatchException;
import com.example.simple_money_transfers.exception.InactiveAccountException;
import com.example.simple_money_transfers.exception.InsufficientFundsException;
import com.example.simple_money_transfers.exception.InvalidMoneyException;
import com.example.simple_money_transfers.exception.NotFoundException;
import com.example.simple_money_transfers.exception.SelfTransferException;
...
import com.example.simple_money_transfers.repository.TransferRepository;

/**
 * Fast, Spring-free coverage of every rejection branch in
 * {@link TransferService#execute}, with mocked repositories - complements
 * {@link TransferServiceIT}'s H2-backed happy-path and atomicity coverage.
 */
...
class TransferServiceTest {
	...
	void insufficientFundsIsRejectedForACustomerSource() {
		UUID sourceId = UUID.randomUUID();
		UUID targetId = UUID.randomUUID();
		Account source = account(sourceId, AccountType.CUSTOMER, "USD", AccountStatus.ACTIVE, new BigDecimal("5.0000"));
		Account target = account(targetId, AccountType.CUSTOMER, "USD", AccountStatus.ACTIVE, new BigDecimal("0.0000"));
		when(accountRepository.lockAllById(List.of(sourceId, targetId))).thenReturn(List.of(source, target));
	...
	void systemSourceIsExemptFromInsufficientFundsCheck() {
		UUID sourceId = UUID.randomUUID();
		UUID targetId = UUID.randomUUID();
		Account source = account(sourceId, AccountType.SYSTEM, "USD", AccountStatus.ACTIVE, new BigDecimal("0.0000"));
		Account target = account(targetId, AccountType.CUSTOMER, "USD", AccountStatus.ACTIVE, new BigDecimal("0.0000"));
		when(accountRepository.lockAllById(List.of(sourceId, targetId))).thenReturn(List.of(source, target));
		when(transferRepository.save(any())).thenAnswer(invocation -> invocation.getArgument(0));

		transferService.execute(
				new TransferCommand(sourceId, targetId, new BigDecimal("50.00"), "USD", TransferKind.DEPOSIT, null));
