...
class ApiExceptionHandlerTest {
  ...
  void businessRuleExceptionMapsTo422() throws Exception {
    mockMvc
        .perform(post("/probe/business-rule"))
  ...
  void malformedJsonMapsTo400() throws Exception {
    mockMvc
        .perform(
            post("/probe/validated").contentType(MediaType.APPLICATION_JSON).content("{not json"))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentType(MediaType.APPLICATION_PROBLEM_JSON))
  ...
  void malformedPathVariableMapsTo400() throws Exception {
    mockMvc
        .perform(get("/probe/type-mismatch/not-a-uuid"))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentType(MediaType.APPLICATION_PROBLEM_JSON))
