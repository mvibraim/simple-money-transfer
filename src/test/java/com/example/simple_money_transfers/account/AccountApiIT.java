...
class AccountApiIT extends AbstractIntegrationTest {
  ...
  void createThenFetchRoundTrip() throws Exception {
    ...
    String id =
        jsonMapper.readTree(created.getResponse().getContentAsString()).path("id").asString();

    mockMvc
        .perform(get("/api/v1/accounts/" + id).header(ApiKeyAuthFilter.HEADER_NAME, VALID_KEY))
  ...
  void unknownAccountIdReturns404() throws Exception {
    mockMvc
        .perform(
            get("/api/v1/accounts/" + java.util.UUID.randomUUID())
                .header(ApiKeyAuthFilter.HEADER_NAME, VALID_KEY))
  ...
  void malformedAccountIdReturns400() throws Exception {
    mockMvc
        .perform(get("/api/v1/accounts/not-a-uuid").header(ApiKeyAuthFilter.HEADER_NAME, VALID_KEY))
        .andExpect(status().isBadRequest());
  }
  ...
  void missingApiKeyIsRejectedOnEveryEndpoint() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/accounts/" + java.util.UUID.randomUUID()))
        .andExpect(status().isUnauthorized());
  }
}
