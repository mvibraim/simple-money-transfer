...
import com.example.simple_money_transfers.exception.IdempotencyConflictException;
...
import com.example.simple_money_transfers.model.dto.TransferCommand;
...
import com.example.simple_money_transfers.repository.IdempotencyRecordRepository;
import com.example.simple_money_transfers.util.RequestFingerprint;

/**
 * Makes every money-moving endpoint safe to retry. The claim row lives in the same
 * transaction as the transfer it belongs to (see {@link IdempotentTransferAttempt}), not
 * a separate "IN_PROGRESS" pre-claim: because both H2 and Postgres block a second
 * inserter on the unique index until the first transaction resolves, concurrency is
 * handled by the database's own locking rather than application-level state. A business
 * rejection rolls back the claim along with the transfer, so the key stays reusable -
 * only successful transfers are ever memoized.
 * <p>
 * Idempotency records are never reaped. A TTL here would be a live double-spend window:
 * delete a record, and a client's legitimate retry of that key executes a second, real
 * transfer.
 */
@Component
public class TransferOrchestrator {
	...
	@Transactional(propagation = Propagation.NEVER)
	public ResponseEntity<String> deposit(String clientId, String idempotencyKey, UUID accountId,
			DepositRequest request) {
		String fingerprint = depositOrWithdrawalFingerprint("DEPOSIT", accountId, request.amount(),
				request.reference());
	...
	@Transactional(propagation = Propagation.NEVER)
	public ResponseEntity<String> withdraw(String clientId, String idempotencyKey, UUID accountId,
			WithdrawalRequest request) {
		String fingerprint = depositOrWithdrawalFingerprint("WITHDRAWAL", accountId, request.amount(),
				request.reference());
		return attemptOrReplay(clientId, idempotencyKey, fingerprint,
				() -> transferService.withdraw(accountId, request.amount(), request.reference()));
	}

	private static String depositOrWithdrawalFingerprint(String kind, UUID accountId, BigDecimal amount,
			String reference) {
