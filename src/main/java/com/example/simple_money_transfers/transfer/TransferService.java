...
/**
 * The single place in the codebase that ever mutates an account balance or writes a ledger entry.
 * See the F09 design doc for why locking is ordered and pessimistic - the short version: {@link
 * AccountRepository#lockAllById} acquires both accounts in one query, ascending by id, which is
 * what makes two opposing concurrent transfers resolve to a wait instead of a deadlock. That
 * guarantee holds only as long as no other code path locks an account outside that one query.
 */
@Service
public class TransferService {
  ...
  public Transfer execute(TransferCommand command) {
    ...
    if (!source.getCurrency().equals(command.currency())
        || !target.getCurrency().equals(command.currency())) {
      throw new CurrencyMismatchException(
          command.currency(), source.getCurrency(), target.getCurrency());
    }
    ...
    if (source.getAccountType() == AccountType.CUSTOMER
  ...
  /**
   * Always in the account's own currency - there is no FX conversion, so asking the caller to
   * restate the currency would only invite a spurious mismatch. Reuses {@link #execute}, so this
   * funding path gets every invariant (locking, atomicity, the balanced ledger) that the core
   * transfer already established, for free.
   */
  @Transactional
  ...
  private Account systemAccountFor(String currency) {
    return accountRepository
        .findByAccountTypeAndCurrency(AccountType.SYSTEM, currency)
