...
/**
 * A separate bean from {@link TransferOrchestrator} on purpose: Spring's transactional proxy only
 * intercepts calls that arrive from a different bean, so {@code @Transactional} here would be
 * silently skipped if this method lived on the orchestrator and were called via {@code this.}.
 *
 * <p>{@code action} and the idempotency claim insert run in one transaction. If the claim insert's
 * flush fails on the unique index, this whole transaction rolls back - including whatever {@code
 * action} already did (locking accounts, mutating balances, writing ledger entries) - so a losing
 * concurrent duplicate never leaves a partial transfer behind.
 */
@Component
class IdempotentTransferAttempt {
  ...
  IdempotentTransferAttempt(
      IdempotencyRecordRepository idempotencyRecordRepository, JsonMapper jsonMapper) {
