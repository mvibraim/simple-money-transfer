...
public class ApiExceptionHandler {
  ...
  public ProblemDetail handleTypeMismatch(MethodArgumentTypeMismatchException ex) {
    return ProblemDetail.forStatusAndDetail(
        HttpStatus.BAD_REQUEST, "Malformed value for parameter '" + ex.getName() + "'");
  }

  /**
   * Pessimistic locking (F09) turns one slow or stuck transaction into held row locks; without this
   * mapping, pool exhaustion or a lock/ statement timeout under contention would fall through to
   * the generic 500 handler below and look like an application bug rather than a temporary capacity
   * limit a client can reasonably retry.
   */
  @ExceptionHandler({
